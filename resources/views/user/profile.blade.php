            ...
            <div class="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl border border-gray-100 p-8 mb-8">
                <div class="flex flex-col md:flex-row items-center md:items-start space-y-6 md:space-y-0 md:space-x-8">
                    ...
                    <div class="relative">
                        ...
                        <img src="{{ asset('storage/' . $user->profile_photo_path) }}" alt="{{ $user->name }}" class="w-32 h-32 rounded-full object-cover border-4 border-pink-primary shadow-lg" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                        @endif
                        <div class="w-32 h-32 bg-gradient-to-br from-pink-primary to-pink-600 rounded-full flex items-center justify-center text-white text-4xl font-bold shadow-lg {{ $user->profile_photo_path ? 'hidden' : '' }}">
                    ...
                    <div class="flex-1 text-center md:text-left">
                        ...
                        <p class="text-xl text-gray-600 mb-4">
                            <span class="text-pink-primary">@</span>{{ $user->username ?? 'N/A' }}
                        </p>
                        ...
                        <a href="{{ route('user.profile.edit') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-pink-600 to-pink-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 ring-2 ring-white/20">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8 pt-8 border-t border-gray-200">
                    <div class="text-center md:text-left">
                        <div class="flex items-center justify-center md:justify-start space-x-2 mb-2">
                            ...
                            <h3 class="text-2xl font-bold text-gray-900">{{ $totalLikes }}</h3>
                        </div>
                    ...
                    <div class="text-center md:text-left">
                        <div class="flex items-center justify-center md:justify-start space-x-2 mb-2">
                            ...
                            <h3 class="text-2xl font-bold text-gray-900">{{ $totalBookmarks }}</h3>
                        </div>
            ...
            <div class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-100 mb-8">
                    ...
                    <nav class="flex -mb-px">
                        ...
                        <button onclick="switchTab('bookmarks')" id="tab-bookmarks" class="tab-button flex-1 px-6 py-4 text-center font-semibold text-gray-600 border-b-2 border-transparent hover:text-pink-primary hover:border-pink-primary transition-all duration-200">
                            <div class="flex items-center justify-center space-x-2">
                                ...
                                <span>Disimpan ({{ $totalBookmarks }})</span>
                            </div>
                        </button>
                ...
                <div class="p-6">
                    ...
                    <div id="content-likes" class="tab-content">
                        @if($likedGaleries->count() > 0)
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            @foreach($likedGaleries as $galery)
                            <a href="{{ route('guest.galeri.show', $galery) }}" class="group bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                @if($galery->fotos->count() > 0)
                                <div class="relative h-48 overflow-hidden bg-gray-100">
                                    <img src="{{ asset('storage/' . $galery->fotos->first()->file) }}" alt="{{ $galery->post->judul ?? 'Galeri' }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                        ...
                        <div class="text-center py-12">
                            ...
                            <a href="{{ route('guest.galeri') }}" class="inline-flex items-center px-6 py-3 bg-pink-600 text-white font-bold rounded-xl hover:bg-pink-700 shadow-lg transition-all duration-200 ring-2 ring-white/20">
                                Jelajahi Galeri
                            </a>
                        </div>
                        @endif
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-shadow">
                                <div class="flex items-start space-x-4">
                                    @if($comment->galery && $comment->galery->fotos->count() > 0)
                                    <a href="{{ route('guest.galeri.show', $comment->galery) }}" class="flex-shrink-0">
                                        <img src="{{ asset('storage/' . $comment->galery->fotos->first()->file) }}" alt="Galeri" class="w-20 h-20 rounded-lg object-cover border-2 border-gray-200 hover:border-pink-primary transition-colors" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between mb-2">
                                            <h4 class="font-semibold text-gray-900">
                                                @if($comment->galery && $comment->galery->post)
                                                    <a href="{{ route('guest.galeri.show', $comment->galery) }}" class="hover:text-pink-primary transition-colors">
                                                        {{ $comment->galery->post->judul }}
                                                    </a>
                                                @else
                                        ...
                                        <div class="mt-3 flex items-center space-x-4 text-sm text-gray-500">
                                            ...
                                            @if($comment->galery)
                                            <a href="{{ route('guest.galeri.show', $comment->galery) }}" class="text-pink-primary hover:text-pink-dark font-medium">
                                                Lihat Galeri →
                                            </a>
                                            @endif
                                        </div>
