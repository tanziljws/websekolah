...
class BrevoMailService
{
    ...
    /**
     * Send reset password OTP email via Brevo API
     */
    public function sendResetPasswordOtpEmail(string $to, string $otp): bool
    {
        try {
            $html = View::make('emails.reset-password-otp', ['otp' => $otp])->render();
            ...
            Log::info('Attempting to send reset password OTP email via Brevo API', [
                'to' => $to,
                'from' => $fromAddress,
                'from_name' => $fromName,
                'api_url' => $this->apiUrl
            ...
            $response = Http::timeout(30)->withHeaders([
                ...
                'subject' => 'Kode OTP Reset Password Akun Anda',
                'htmlContent' => $html,
            ]);

            if ($response->successful()) {
                $result = $response->json();
                Log::info('Reset password OTP email sent successfully via Brevo API', [
                    'to' => $to,
                    'message_id' => $result['messageId'] ?? null,
                    'from' => $fromAddress
                ]);
                ...
                Log::error('Brevo API returned error for reset password OTP', [
                    'to' => $to,
                    'status' => $statusCode,
                    'error_message' => $errorMessage,
                    'full_response' => $errorBody,
            ...
            Log::error('Failed to send reset password OTP email via Brevo API', [
                'to' => $to,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
                'from_address' => config('mail.from.address'),
